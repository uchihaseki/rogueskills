...
import { clamp, hashString, round } from "../shared/random.js";
...
export function candidateToSkillGenome(candidate) {
  ...
  const hasSkillArtifact = snapshot.artifactPaths?.some((path) => /(^|\/)skill\.md$/i.test(path));
  const genome = convertMaterialToSkill({
    title: hasSkillArtifact ? null : candidate.name,
    content,
    kind: candidate.kind,
