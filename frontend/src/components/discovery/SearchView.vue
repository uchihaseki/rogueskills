...
<template>
  ...
  <section class="search-console"><div class="search-box"><span>⌕</span><input id="discovery-query" v-model="controller.state.query" placeholder="描述能力、业务目标或失败模式…" autocomplete="off" @keydown.enter="controller.runSearch"><button :disabled="controller.state.searching" @click="controller.runSearch">{{ controller.state.searching ? '正在检索…' : '跨源搜索' }}<b>→</b></button></div><div class="query-examples"><span>查询语法</span><button v-for="query in examples" :key="query" @click="controller.state.query = query">{{ query }}</button></div></section>
