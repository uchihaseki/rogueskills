...
<template>
  <section class="subpage-heading finance-heading">
    <div><p>FINANCE SKILL BOOTSTRAP</p><h1>自动构建金融股票分析技能库</h1><span>从 GitHub 社区召回高质量材料，经过许可证、社区信号、金融相关度和内容安全筛选；金融流程使用 Qwen 归一化后进入同一准入流程。</span></div>
  ...
  <section class="finance-pipeline">
    ...
    <div><b>02</b><strong>自动筛选</strong><span>星标、许可证、相关度、风险</span></div><i>→</i>
    <div><b>03</b><strong>Qwen 标准化</strong><span>社区材料与内置金融 SOP</span></div><i>→</i>
    <div><b>04</b><strong>准入入库</strong><span>结构校验、基准评估、初始技能库</span></div>
  </section>

  <section class="finance-launch" :class="{ 'is-running': controller.state.financeRunning }">
    <div><span>FINANCIAL STOCK ANALYSIS</span><h2>一键初始化金融技能库</h2><p>默认选取最多 2 个社区候选，并转换“基本面与估值分析”“财报与业绩电话会复盘”两份标准流程。该操作会调用 GitHub 和后端配置的 Qwen 模型；点击按钮同时表示允许所有通过许可证、安全和准入硬门槛的结果批量进入初始技能库。</p></div>
  ...
  <template v-if="controller.state.financeResult">
    <section class="finance-summary">
      ...
      <div class="success"><strong>{{ controller.state.financeResult.summary.initialSkills }}</strong><span>初始技能</span></div>
    ...
    <section class="finance-results-grid">
      ...
      <article class="finance-result-panel"><div class="section-heading compact"><div><span>NORMALIZED SOPS</span><h2>金融标准流程</h2></div></div>
