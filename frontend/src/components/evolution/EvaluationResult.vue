...
<template>
  <div v-if="result && !result.kind" class="evaluation-result" :class="result.passed ? 'passed' : 'failed'">
    <div class="result-verdict"><span>{{ result.passed ? 'PASS' : 'FAIL' }}</span><div><small>BUSINESS COVERAGE</small><strong>{{ result.coverage }}% <i>/ {{ result.threshold }}%</i></strong></div></div>
    ...
    <p v-if="!result.securityGatePassed" class="security-warning">安全门槛未通过：构筑缺少应对恶意输入或权限边界的能力。</p>
  </div>
</template>
